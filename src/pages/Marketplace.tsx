...
function TemplateCard({
  ...
  return (
    ...
    >
      <Card
        className="group cursor-pointer overflow-hidden border-border/50 hover:shadow-lg hover:border-primary/30 transition-all duration-300"
        onClick={onSelect}
      >
        ...
        <CardContent className="p-5 space-y-3">
          ...
          <div className="flex items-center gap-3 text-xs text-muted-foreground">
            <span className="flex items-center gap-1">
              ...
              {formatCount(template.usageCount)} ใช้งาน
            </span>
          ...
          <Button
            size="sm"
            className="w-full gradient-primary text-primary-foreground gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity"
            onClick={(e) => {
              e.stopPropagation();
              onClone();
            }}
          >
            ใช้ Template นี้ <ArrowRight className="h-3.5 w-3.5" />
          </Button>
        </CardContent>
      </Card>
    </motion.div>
...
export default function Marketplace() {
  ...
  const featured = useMemo(
    () => MARKETPLACE_TEMPLATES.filter((t) => t.featured),
    []
  );
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-display font-bold gradient-text">Template Marketplace</h1>
        <p className="text-muted-foreground mt-1">
          เลือก template สำเร็จรูปแล้วสร้าง Agent ได้ทันทีในคลิกเดียว
        </p>
      </div>
      ...
      <section className="space-y-3">
        <h2 className="text-lg font-semibold">
          {category === "All" ? "Template ทั้งหมด" : `หมวด ${category}`}{" "}
        ...
        {filtered.length === 0 && (
          <p className="text-center text-muted-foreground py-12">ไม่พบ template ที่ตรงกับการค้นหา</p>
        )}
      </section>
          ...
          <DialogContent className="max-w-lg">
            ...
            <div className="space-y-4">
              <p className="text-sm text-foreground/80 leading-relaxed">
                {selected.previewDescription}
              </p>

              <div className="flex items-center gap-4">
                <StarRating rating={selected.rating} />
                <span className="text-xs text-muted-foreground">
                  ({selected.reviewCount} รีวิว)
                </span>
                <span className="text-xs text-muted-foreground flex items-center gap-1">
                  <Users className="h-3 w-3" /> {formatCount(selected.usageCount)} ใช้งาน
                </span>
              </div>

              <div>
                <p className="text-xs font-medium text-muted-foreground mb-1">เครื่องมือที่ใช้</p>
              ...
              <div>
                <p className="text-xs font-medium text-muted-foreground mb-1">โมเดลแนะนำ</p>
              ...
              >
                ใช้ Template นี้ <ArrowRight className="h-4 w-4" />
              </Button>
            </div>
          </DialogContent>
        )}
